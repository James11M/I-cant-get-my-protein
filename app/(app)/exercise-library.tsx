...
import { DEFAULT_EQUIPMENT, EquipmentPreferences, getEquipmentPreferences, getExerciseFavourites, saveEquipmentPreferences, setExerciseFavourite } from '@/features/exercises/exercise-library.service';
...
export default function ExerciseLibraryScreen() {
  ...
  const [exercises, setExercises] = useState<VisualExercise[]>(POC_EXERCISES);
  ...
  useFocusEffect(useCallback(() => {
    ...
    async function load() {
      setLoading(true);
      const [savedFavourites, savedEquipment, visual] = await Promise.all([
        getExerciseFavourites().catch(() => []),
        getEquipmentPreferences().catch(() => DEFAULT_EQUIPMENT),
        loadVisualExercises(POC_EXERCISES),
      ]);
      if (!live) return;
    ...
    <Screen>
      ...
      <Card style={styles.equipmentCard}>
        <Pressable style={styles.rowBetween} onPress={() => setEquipmentOpen((value) => !value)}>
          <View style={styles.flex}>
            ...
            {!equipmentOpen ? <Text style={styles.muted}>{availableLabels.join(' • ')}</Text> : null}
          </View>
          <CardAction colour={colours.gold} label={equipmentOpen ? 'HIDE ↑' : 'VIEW →'} />
        </Pressable>
        {equipmentOpen ? <View style={styles.equipmentList}>
          {EQUIPMENT.map(([key, label], index) => (
            <Pressable key={key} style={[styles.equipmentRow, index < EQUIPMENT.length - 1 && styles.divider]} onPress={() => toggleEquipment(key)}>
              <Text style={styles.equipmentName}>{label}</Text>
              <View style={[styles.toggle, equipment[key] && styles.toggleOn]}><View style={[styles.knob, equipment[key] && styles.knobOn]} /></View>
            </Pressable>
          ))}
        </View> : null}
      </Card>

      <Pressable style={styles.addOwn}><Text style={styles.addOwnText}>+ ADD YOUR OWN EXERCISE</Text></Pressable>
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.filters} style={styles.filterScroll}>
        {SELECTORS.map(([key, label]) => (
          <Pressable key={key} style={[styles.filter, selector === key && styles.filterOn]} onPress={() => setSelector(key)}>
            <Text style={[styles.filterText, selector === key && styles.filterTextOn]} numberOfLines={1}>{label}</Text>
          </Pressable>
        ))}
      </ScrollView>
        ...
        <View key={exercise.id} style={styles.exerciseCard}>
          <Pressable style={styles.exerciseMain} onPress={() => router.push({ pathname: '/(app)/exercise-detail', params: { id: exercise.id } })}>
            ...
            <View style={styles.flex}>
              ...
              <View style={styles.equipmentPill}><Text style={styles.equipmentPillText}>{EQUIPMENT_LABELS[exercise.equipment]}</Text></View>
              <Text style={styles.exerciseMeta}>{exercise.group} • {exercise.inputType === 'time' ? 'Timed' : 'Reps'}</Text>
            </View>
            <Text style={styles.arrow}>→</Text>
          </Pressable>
          <Pressable style={styles.favouriteStrip} onPress={() => toggleFavourite(exercise.id)}>
            <Text style={[styles.star, favourites.includes(exercise.id) ? styles.starOn : styles.starOff]}>{favourites.includes(exercise.id) ? '★' : '☆'}</Text>
          </Pressable>
        </View>
      ))}
...
const styles = StyleSheet.create({
  title: { color: colours.white, fontSize: 30, lineHeight: 34, fontWeight: '900', marginBottom: 14 },
  flex: { flex: 1 },
  rowBetween: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  sectionTitle: { color: colours.white, fontSize: 11, fontWeight: '900', letterSpacing: 0.7 },
  equipmentCard: { borderColor: colours.border, marginBottom: 12 },
  muted: { color: colours.muted, fontSize: 10, marginTop: 6 },
  equipmentList: { marginTop: 10, borderTopWidth: 1, borderTopColor: colours.border },
  equipmentRow: { minHeight: 64, flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  divider: { borderBottomWidth: 1, borderBottomColor: colours.border },
  equipmentName: { color: colours.white, fontSize: 15, fontWeight: '900' },
  toggle: { width: 52, height: 30, borderRadius: 15, padding: 3, backgroundColor: colours.card2, justifyContent: 'center' },
  toggleOn: { backgroundColor: colours.gold },
  knob: { width: 24, height: 24, borderRadius: 12, backgroundColor: colours.muted },
  knobOn: { alignSelf: 'flex-end', backgroundColor: colours.background },
  addOwn: { minHeight: 50, borderWidth: 1, borderColor: colours.gold, borderRadius: 11, alignItems: 'center', justifyContent: 'center', marginBottom: 12 },
  addOwnText: { color: colours.gold, fontSize: 11, fontWeight: '900', letterSpacing: 0.5 },
  search: { minHeight: 54, backgroundColor: colours.card, borderWidth: 1, borderColor: colours.border, borderRadius: 12, color: colours.white, paddingHorizontal: 14, fontSize: 15, marginBottom: 9 },
  filterScroll: { marginHorizontal: -1, marginBottom: 12 },
  filters: { gap: 8, paddingHorizontal: 1, paddingRight: 16 },
  filter: { minHeight: 38, borderWidth: 1, borderColor: colours.border, borderRadius: 20, paddingHorizontal: 15, alignItems: 'center', justifyContent: 'center' },
  filterOn: { backgroundColor: colours.gold, borderColor: colours.gold },
  filterText: { color: colours.muted, fontSize: 9, fontWeight: '900' },
  filterTextOn: { color: colours.background },
  ...
  exerciseMeta: { color: colours.muted, fontSize: 10, marginTop: 6 },
  arrow: { color: colours.gold, fontSize: 28, marginLeft: 8 },
  favouriteStrip: { minHeight: 43, borderTopWidth: 1, borderTopColor: colours.border, alignItems: 'flex-end', justifyContent: 'center', paddingHorizontal: 14 },
  star: { fontSize: 28 },
  starOn: { color: colours.gold },
  starOff: { color: colours.muted },
  attribution: { color: colours.muted, fontSize: 8, textAlign: 'center', marginTop: 2, marginBottom: 12 },
});
