...
type ProteinProfile = { enabled: boolean; goal: string | null; override: number | null; weight: number | null; dob: string | null };
...
export default function HomeScreen() {
  ...
  useFocusEffect(useCallback(() => {
    ...
    async function load() {
      ...
      if (!live || !data) return;
      setProtein({ enabled: Boolean(data.protein_enabled), goal: data.protein_goal || null, override: data.protein_multiplier_override == null ? null : Number(data.protein_multiplier_override), weight: data.current_weight_kg == null ? null : Number(data.current_weight_kg), dob: data.date_of_birth || null });
    }
      ...
      <ScrollView style={styles.scroll} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <Pressable onPress={() => router.push('/(app)/levels')}>
          <Card style={styles.levelCard}>
            <View style={styles.rowBetween}><View style={styles.flex}><Text style={styles.goldLabel}>LEVEL {rank.level}</Text><Text style={styles.levelTitle}>{rank.title}</Text><Text style={styles.activeXp}>{activeXP.toLocaleString()} ACTIVE XP</Text>{rank.next ? <Text style={styles.nextLevel}>{rank.xpToNext.toLocaleString()} XP → {rank.next.title}</Text> : null}</View><View style={styles.levelCircle}><Text style={styles.levelNumber}>{rank.level}</Text></View></View>
        ...
        <Card style={styles.targetCard}>
          <View style={styles.rowBetween}><View><Text style={styles.sectionTitle}>TODAY'S TARGET</Text><Text style={[styles.targetValue, complete && styles.targetValueComplete]}>{Math.round(rawTodayCredit)} / 30</Text><Text style={styles.smallMuted}>credit minutes</Text></View><ProgressRing minutes={rawTodayCredit} target={DAILY_TARGET} /></View>
          ...
          <View style={styles.rewardPanel}><View><Text style={styles.rewardLabel}>{complete ? 'XP EARNED' : 'WHEN COMPLETE'}</Text><Text style={styles.rewardXp}>+{rewardXP} XP</Text></View><View style={styles.rewardRight}><Text style={styles.multiplier}>×{multiplier.toFixed(2)}</Text><Text style={styles.rewardSub}>consistency boost</Text></View></View>
          <View style={styles.fireSection}><View style={styles.flex}><Text style={styles.fireLabel}>7-DAY FIRE SCORE</Text><View style={styles.fireRow}>{fireWindow.map((done, index) => <View key={index} style={[styles.fireWrap, index === 6 && styles.fireToday]}><Text style={[styles.fireEmoji, !done && styles.fireOff]}>🔥</Text></View>)}</View></View><Text style={styles.fireCount}>{fireScore}</Text></View>
        ...
        {protein.enabled ? (
          <Pressable style={styles.proteinPress} onPress={() => router.push(proteinRoute)}>
            <Card style={styles.proteinCard}>
              ...
              {proteinTarget && proteinGoalTitle ? <><Text style={styles.levelTitle}>{proteinGoalTitle}</Text><Text style={styles.proteinTarget}>{proteinTarget}g DAILY TARGET</Text><Text style={styles.proteinMeta}>{protein.weight} kg × {proteinMultiplier?.toFixed(1)} g/kg</Text></> : <><Text style={styles.proteinValue}>{!protein.goal ? 'SET YOUR GOAL' : 'ADD YOUR WEIGHT'}</Text><Text style={styles.proteinMeta}>{!protein.goal ? 'Choose a protein goal to calculate your target.' : 'Add Weight in Stats & Measurements to calculate your target.'}</Text></>}
            </Card>
          </Pressable>
        ) : null}
      </ScrollView>
...
function getRollingComeback(logs: ActivityLog[], today: Date) {
  const windowStart = addDays(today, -6); const deficits: { key: string; remaining: number }[] = []; const recoveredByDate: Record<string, number> = {}; let totalExtra = 0; let recovered = 0;
  for (let index = 0; index < 7; index += 1) { const date = addDays(windowStart, index); const normal = normalCreditForDate(logs, date); const missing = Math.max(0, DAILY_TARGET - normal); const extra = extraCreditForDate(logs, date); totalExtra += extra; let available = extra * COMEBACK_RATE; for (let deficitIndex = deficits.length - 1; deficitIndex >= 0 && available > 0; deficitIndex -= 1) { const deficit = deficits[deficitIndex]; if (deficit.remaining <= 0) continue; const applied = Math.min(available, deficit.remaining); deficit.remaining -= applied; available -= applied; recovered += applied; recoveredByDate[deficit.key] = (recoveredByDate[deficit.key] || 0) + applied; } if (missing > 0) deficits.push({ key: dateKey(date), remaining: missing }); }
  return { recoveredByDate, totalExtra, recovered, missing: deficits.reduce((sum, item) => sum + item.remaining, 0) };
}
function effectiveCredit(logs: ActivityLog[], date: Date, today: Date) { const comeback = getRollingComeback(logs, today); return Math.min(DAILY_TARGET, normalCreditForDate(logs, date) + (comeback.recoveredByDate[dateKey(date)] || 0)); }
function effectiveComplete(logs: ActivityLog[], date: Date, today: Date) { return effectiveCredit(logs, date, today) >= DAILY_TARGET; }
function getFireWindow(logs: ActivityLog[], today: Date) { return Array.from({ length: 7 }, (_, index) => effectiveComplete(logs, addDays(today, index - 6), today)); }
function getRank(xp: number) { let current = LEVELS[0]; let next: typeof LEVELS[number] | null = LEVELS[1]; LEVELS.forEach((level, index) => { if (xp >= level[2]) { current = level; next = LEVELS[index + 1] || null; } }); if (!next) return { level: current[0], title: current[1], next: null, xpToNext: 0, progress: 100 }; const progress = Math.max(0, Math.min(100, Math.round(((xp - current[2]) / (next[2] - current[2])) * 100))); return { level: current[0], title: current[1], next: { title: next[1] }, xpToNext: next[2] - xp, progress }; }
...
const styles = StyleSheet.create({
  screen: { paddingBottom: 6 }, scroll: { flex: 1 }, scrollContent: { paddingBottom: 10 }, title: { color: colours.white, fontSize: 29, fontWeight: '900', marginTop: 18, marginBottom: 8 }, subtitle: { color: colours.muted, fontSize: 11, lineHeight: 17, marginBottom: 14 }, rowBetween: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center' }, cardTitleRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', marginBottom: 2 }, flex: { flex: 1 }, goldLabel: { color: colours.gold, fontSize: 9, fontWeight: '900' }, sectionTitle: { color: colours.white, fontSize: 11, fontWeight: '900', letterSpacing: 0.4 }, smallMuted: { color: colours.muted, fontSize: 9, lineHeight: 14, marginTop: 3 }, tinyMuted: { color: colours.muted, fontSize: 7, marginTop: 5 },
