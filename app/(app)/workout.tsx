...
type WorkoutExercise = { definition: PickerExercise; sets: WorkoutSet[]; view: 'sets' | 'howto' };
...
export default function WorkoutScreen() {
  ...
  useEffect(() => {
    Promise.all([getStrengthExercises(), getEquipmentPreferences(), getExerciseFavourites()]).then(async ([definitions, prefs, favs]) => {
      ...
      setLibrary(visual as PickerExercise[]);
      setEquipment(prefs);
      setFavourites(favs);
    }).catch(() => {});
  }, [adding]);
  ...
  function addExercise(definition: PickerExercise) {
    setWorkout((current) => [...current, { definition, sets:Array.from({length:definition.defaultSets},()=>blankSet(definition)), view:'sets' }]);
    setAdding(false);
    setQuery('');
  }
  function setExerciseView(exerciseIndex:number, view:'sets'|'howto') {
    setWorkout((current)=>current.map((exercise,index)=>index===exerciseIndex?{...exercise,view}:exercise));
  }
  function updateSet(exerciseIndex:number,setIndex:number,field:keyof WorkoutSet,value:string){
    setWorkout((current)=>current.map((exercise,index)=>index!==exerciseIndex?exercise:{...exercise,sets:exercise.sets.map((set,inner)=>inner===setIndex?{...set,[field]:value}:set)}));
  }
  function addSet(exerciseIndex:number){
    setWorkout((current)=>current.map((exercise,index)=>index!==exerciseIndex?exercise:{...exercise,sets:[...exercise.sets,blankSet(exercise.definition)]}));
  }
  function removeSet(exerciseIndex:number){
    setWorkout((current)=>current.map((exercise,index)=>index!==exerciseIndex||exercise.sets.length<=1?exercise:{...exercise,sets:exercise.sets.slice(0,-1)}));
  }
  ...
  async function finishWorkout(){
    if(workout.length===0){setComplete(true);return;}
    setSaving(true);
    try{
      if(session?.user.id)await addSimpleActivityLog({userId:session.user.id,name:'Strength Training',icon:'🏋️',trainingType:'Strength',durationMinutes:minutes,creditMinutes:minutes,...(date?{performedAt:`${date}T12:00:00`}:{})});
      setComplete(true);
  ...
  return <Screen><Brand/><BackButton/><Text style={styles.title}>Workout</Text><Text style={styles.subtitle}>{date?`${formatDate(date)} • `:''}Estimated duration: {minutes} min</Text>
    {workout.map((exercise,exerciseIndex)=><Card key={`${exercise.definition.id}-${exerciseIndex}`} style={styles.exerciseCard}>
      <View style={styles.exerciseHeader}><View style={styles.flex}><Text style={styles.exerciseTitle}>{exercise.definition.name}</Text><Text style={styles.meta}>{EQUIPMENT_LABELS[exercise.definition.equipment]} • {exercise.definition.measurements.map((m)=>m.toUpperCase()).join(' + ')}</Text></View><Pressable onPress={()=>removeExercise(exerciseIndex)}><Text style={styles.removeExercise}>×</Text></Pressable></View>
      <View style={styles.modeRow}>
        <Pressable style={[styles.modePill,exercise.view==='sets'&&styles.modePillOn]} onPress={()=>setExerciseView(exerciseIndex,'sets')}><Text style={[styles.modeText,exercise.view==='sets'&&styles.modeTextOn]}>SETS</Text></Pressable>
        <Pressable style={[styles.modePill,exercise.view==='howto'&&styles.modePillOn]} onPress={()=>setExerciseView(exerciseIndex,'howto')}><Text style={[styles.modeText,exercise.view==='howto'&&styles.modeTextOn]}>HOW TO</Text></Pressable>
      </View>
      ...
      {exercise.view==='sets'?<>
        ...
        <View style={styles.setActions}>{exercise.sets.length>1?<Pressable onPress={()=>removeSet(exerciseIndex)}><Text style={styles.removeSetText}>✕ REMOVE SET</Text></Pressable>:<View/>}<Pressable onPress={()=>addSet(exerciseIndex)}><Text style={styles.addSetText}>+ ADD SET</Text></Pressable></View>
      </>:<View style={styles.howToCard}>
        <View style={styles.howToVisual}><ExerciseVisual exercise={exercise.definition}/></View>
        {(exercise.definition.instructions||[]).length?<View style={styles.instructions}><Text style={styles.howToTitle}>TECHNIQUE</Text>{(exercise.definition.instructions||[]).map((instruction,index)=><Text key={index} style={styles.instruction}>{index+1}. {instruction}</Text>)}</View>:<Text style={styles.howToFallback}>Use controlled technique and stop if form breaks down.</Text>}
        {exercise.definition.videoUrl?<Pressable style={styles.videoButton} onPress={()=>Linking.openURL(exercise.definition.videoUrl!)}><Text style={styles.videoButtonText}>▶ WATCH VIDEO</Text></Pressable>:null}
      </View>}
    </Card>)}
...
const styles=StyleSheet.create({
  flex:{flex:1},title:{color:colours.white,fontSize:31,fontWeight:'900'},subtitle:{color:colours.muted,fontSize:11,fontWeight:'700',marginTop:5,marginBottom:18},sectionTitle:{color:colours.white,fontSize:11,fontWeight:'900',letterSpacing:.6},
  exerciseCard:{marginBottom:10},exerciseHeader:{flexDirection:'row',alignItems:'center',marginBottom:10},exerciseTitle:{color:colours.white,fontSize:15,fontWeight:'900'},meta:{color:colours.muted,fontSize:8,fontWeight:'800',marginTop:4},removeExercise:{color:colours.muted,fontSize:26,paddingHorizontal:5},
  modeRow:{flexDirection:'row',gap:7,marginBottom:12},modePill:{flex:1,minHeight:36,borderWidth:1,borderColor:colours.border,borderRadius:18,alignItems:'center',justifyContent:'center',backgroundColor:colours.card2},modePillOn:{backgroundColor:colours.gold,borderColor:colours.gold},modeText:{color:colours.muted,fontSize:9,fontWeight:'900',letterSpacing:.6},modeTextOn:{color:colours.background},
  setHead:{flexDirection:'row',alignItems:'center',marginBottom:4},setHeadText:{color:colours.muted,fontSize:7,fontWeight:'900',textAlign:'center'},setNumber:{width:38},setField:{flex:1},setRow:{flexDirection:'row',alignItems:'center',marginTop:6},setIndex:{color:colours.white,fontSize:12,fontWeight:'900',textAlign:'center'},setInput:{backgroundColor:colours.card2,borderWidth:1,borderColor:colours.border,borderRadius:8,color:colours.white,minHeight:38,marginHorizontal:3,paddingHorizontal:6,textAlign:'center',fontWeight:'900'},
  setActions:{borderTopWidth:1,borderTopColor:colours.border,marginTop:12,paddingTop:10,flexDirection:'row',alignItems:'center',justifyContent:'space-between'},removeSetText:{color:colours.red,fontSize:9,fontWeight:'900'},addSetText:{color:colours.gold,fontSize:9,fontWeight:'900'},
  howToCard:{borderWidth:1,borderColor:colours.border,borderRadius:12,backgroundColor:colours.card2,padding:10},howToVisual:{height:260,borderRadius:10,overflow:'hidden',backgroundColor:colours.card,marginBottom:10},instructions:{borderTopWidth:1,borderTopColor:colours.border,paddingTop:10},howToTitle:{color:colours.gold,fontSize:9,fontWeight:'900',letterSpacing:.7},instruction:{color:colours.white,fontSize:10,lineHeight:16,marginTop:7},howToFallback:{color:colours.muted,fontSize:10,lineHeight:16},videoButton:{minHeight:42,borderWidth:1,borderColor:colours.gold,borderRadius:10,alignItems:'center',justifyContent:'center',marginTop:12},videoButtonText:{color:colours.gold,fontSize:9,fontWeight:'900',letterSpacing:.7},
  outline:{borderWidth:1,borderColor:colours.gold,borderRadius:13,minHeight:48,alignItems:'center',justifyContent:'center',marginBottom:10},outlineText:{color:colours.gold,fontSize:10,fontWeight:'900',letterSpacing:1},finish:{backgroundColor:colours.gold,borderRadius:13,minHeight:48,alignItems:'center',justifyContent:'center',marginTop:2},finishText:{color:colours.background,fontSize:11,fontWeight:'900',letterSpacing:1.2},disabled:{opacity:.45},
  pickerCard:{borderColor:colours.gold,marginBottom:10},input:{backgroundColor:colours.card2,borderWidth:1,borderColor:colours.border,borderRadius:10,color:colours.white,paddingHorizontal:12,paddingVertical:10,marginTop:10,marginBottom:10},filters:{gap:7,paddingBottom:12,paddingRight:12},filter:{borderWidth:1,borderColor:colours.border,borderRadius:18,paddingHorizontal:11,paddingVertical:7},filterOn:{backgroundColor:colours.gold,borderColor:colours.gold},filterText:{color:colours.muted,fontSize:8,fontWeight:'900'},filterTextOn:{color:colours.background},libraryCard:{borderWidth:1,borderColor:colours.border,borderRadius:12,overflow:'hidden',marginBottom:8},libraryMain:{minHeight:94,flexDirection:'row',alignItems:'center',padding:9},thumb:{width:82,height:78,borderRadius:8,overflow:'hidden',backgroundColor:colours.card2,marginRight:10},choiceTitle:{color:colours.white,fontSize:12,fontWeight:'900'},arrow:{color:colours.gold,fontSize:24,fontWeight:'900',marginLeft:8},starStrip:{borderTopWidth:1,borderTopColor:colours.border,alignItems:'flex-end',paddingHorizontal:12,paddingVertical:5},star:{color:colours.muted,fontSize:24},starOn:{color:colours.gold},cancel:{alignItems:'center',paddingTop:8},cancelText:{color:colours.muted,fontSize:9,fontWeight:'900'},
  completeHero:{alignItems:'center',paddingVertical:20},completeEmoji:{fontSize:42},completeTitle:{color:colours.gold,fontSize:18,fontWeight:'900',marginTop:8,letterSpacing:1},completeDate:{color:colours.white,fontSize:10,fontWeight:'900',marginTop:5},statsRow:{flexDirection:'row',justifyContent:'space-around'},stat:{alignItems:'center',flex:1},statValue:{color:colours.white,fontSize:25,fontWeight:'900'},statLabel:{color:colours.muted,fontSize:8,fontWeight:'900',marginTop:3},sectionGap:{marginTop:10,marginBottom:10},spacer:{height:12}
});
