  ...
  return (
    <View style={styles.nav}>
      <NavItem
        label="TODAY"
        active={active === 'today'}
        icon={<HomeIcon active={active === 'today'} />}
      ...
      <NavItem
        ...
        icon={<PodiumIcon active={active === 'leaders'} />}
      />

      <NavItem
        label="NERD"
        active={active === 'nerd'}
        icon={<GearIcon active={active === 'nerd'} />}
      />
    </View>
  );
}
...
function HomeIcon({ active }: { active: boolean }) {
  ...
  return (
    <View style={styles.homeBox}>
      <View style={[styles.homeRoof, { borderBottomColor: colour }]} />
      <View style={[styles.homeBody, { backgroundColor: colour }]}>
...
function ClockIcon({ active }: { active: boolean }) {
  return (
    <View style={[styles.clockFace, { backgroundColor: active ? colours.gold : colours.white }]}>
      <View style={styles.clockHandLong} />
      <View style={styles.clockHandShort} />
      <View style={styles.clockCentre} />
    </View>
  );
}
...
function PodiumIcon({ active }: { active: boolean }) {
  ...
  return (
    <View style={styles.podium}>
      <View style={[styles.podiumSide, { backgroundColor: colour }]} />
      <View style={[styles.podiumMiddle, { backgroundColor: colour }]} />
      <View style={[styles.podiumSide, { backgroundColor: colour }]} />
    </View>
  );
}
  ...
  return (
    <View style={styles.gearBox}>
      {Array.from({ length: 8 }, (_, index) => (
        <View key={index} style={[styles.gearToothWrapper, { transform: [{ rotate: `${index * 45}deg` }] }]}>
          <View style={[styles.gearTooth, { backgroundColor: colour }]} />
        </View>
      ))}
      <View style={[styles.gearDisc, { backgroundColor: colour }]}>
...
const styles = StyleSheet.create({
  ...
  podiumSide: { width: 7, height: 12, borderRadius: 1 },
  podiumMiddle: { width: 8, height: 20, borderRadius: 1 },
  ...
  gearTooth: { width: 5, height: 7, borderRadius: 1 },
  gearDisc: { width: 18, height: 18, borderRadius: 9, alignItems: 'center', justifyContent: 'center' },
